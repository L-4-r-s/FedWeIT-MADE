    ...
    def build_model(self):
        ...
        for i in range(1,len(self.units_per_layer)-1): #exclude input & output layer
            if i == 1:
                ...
                x_layers.append(FedweitDirectInputConnectConditionningMaskedLayer(units = self.units_per_layer[i], mask = masks[i-1], masked_layer_id = i-1, use_cond_mask = self.connectivity_weights, kernel_initializer=self.kernel_initializer, global_weights = self.global_weights, adaptive_factor = self.adaptive_factor, num_tasks = self.num_tasks)(x_layers[i-1])) #activation is relu, call custom_layer with previous layer as input-param
